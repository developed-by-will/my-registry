...
`;
  ...
  for (const item of registry) {
    const resolveFiles = item.files?.map(
      (file) =>
        `registry/components/${typeof file === "string" ? file : file.path}`
    );
    if (!resolveFiles) {
      continue;
    }
      ...
      chunks = await Promise.all(
        components.map(async (component, index) => {
          ...
          const targetFilePath = path.join(
            cwd(),
            `registry/components/${type}/${chunkName}.tsx`
          );
...
`;
  ...
  if (!existsSync(path.join(REGISTRY_PATH, "index.json"))) {
    await fs.mkdir(path.join(REGISTRY_PATH, "index.json"), { recursive: true });
  }
  ...
  if (!existsSync(path.join(process.cwd(), "__registry__"))) {
    await fs.mkdir(path.join(process.cwd(), "__registry__"), {
      recursive: true,
    });
    await fs.writeFile(
